<template>
  <div class="nfield">
    ...
    />
    <i class="fas fa-calendar nfield__icon" aria-hidden="true"></i>
  </div>
</template>

<script setup>
/**
 * A date field that opens the platform's own picker.
 *
 * This is deliberately the native control rather than the drawn calendar in
 * BaseDate.vue, which is still what the public booking form uses. On a phone -
 * which is where Caroline writes most invoices - the native control is the
 * scrolling wheel, and picking a date on a wheel with a thumb beats hunting a
 * 7x6 grid of small targets. The value format is unchanged either way:
 * YYYY-MM-DD, which is what `input[type=date]` speaks natively.
 */
defineProps({
  /** YYYY-MM-DD */
...
/* Safari draws its own small calendar glyph, and Chrome a different one, so
