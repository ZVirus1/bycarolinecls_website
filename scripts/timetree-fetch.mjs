 ...
 * TimeTree. The entire thing is ~150 lines and talks to exactly two URLs, both
 * on timetreeapp.com. Nothing else. No dependencies beyond Node 20's built-in
 * fetch, so there is no supply chain to trust.
 *
 * Your password:
...
const output = JSON.stringify(
  {
    ...
    events: events.map(normalise).filter(Boolean),
  },
  null,
  2,
)
